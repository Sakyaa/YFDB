...
// MARK: - YFDatabase Private Extension
...
+ (instancetype)resultSetWithStatement:(YFStatement *)statement usingParentDatabase:(YFDatabase*)aDB shouldAutoClose:(BOOL)shouldAutoClose {
    YFResultSet *rs = [[YFResultSet alloc] init];
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"YFDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
...
- (YFSqliteValueType)typeForColumn:(NSString*)columnName {
    return sqlite3_column_type([_statement statement], [self columnIndexForName:columnName]);
}

- (YFSqliteValueType)typeForColumnIndex:(int)columnIdx {
