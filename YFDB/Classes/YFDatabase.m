...
- (YFResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray * _Nullable)arrayArgs orDictionary:(NSDictionary * _Nullable)dictionaryArgs orVAList:(va_list)args shouldBind:(BOOL)shouldBind;
...
// MARK: - YFResultSet Private Extension

@interface YFResultSet ()

- (int)internalStepWithError:(NSError * _Nullable __autoreleasing *)outErr;
+ (instancetype)resultSetWithStatement:(YFStatement *)statement usingParentDatabase:(YFDatabase*)aDB shouldAutoClose:(BOOL)shouldAutoClose;
...
#pragma mark YFDatabase instantiation and deallocation
...
+ (SInt32)YFDBVersion {
    ...
    static SInt32 YFDBVersionVal = 0;
    
    dispatch_once(&once, ^{
        NSString *prodVersion = [self YFDBUserVersion];
        ...
        for (NSUInteger i = 0; i < 3; i++) {
            ...
            if (component > 15) {
                NSLog(@"YFDBVersion is invalid: Please use YFDBUserVersion instead.");
                component = 15;
            }
            YFDBVersionVal = YFDBVersionVal << 4 | component;
        }
    });
    
    return YFDBVersionVal;
}
...
static int YFDBDatabaseBusyHandler(void *f, int count) {
    YFDatabase *self = (__bridge YFDatabase*)f;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &YFDBDatabaseBusyHandler, (__bridge void *)(self));
    }
    else {
...
// but for folks who don't bother noticing that the interface to YFDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"YFDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"YFDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)resultSetDidClose:(YFResultSet *)resultSet {
...
- (void)clearCachedStatements {
    ...
    for (NSMutableSet *statements in [_cachedStatements objectEnumerator]) {
        for (YFStatement *statement in [statements allObjects]) {
            [statement close];
        }
    }
...
- (YFStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(YFStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(YFStatement*)statement forQuery:(NSString*)query {
    NSParameterAssert(query);
    if (!query) {
        NSLog(@"API misuse, -[YFDatabase setCachedStatement:forQuery:] query must not be nil");
        return;
    }
...
- (BOOL)goodConnection {
    ...
    YFResultSet *rs = [self executeQuery:@"PRAGMA cipher_version"];
...
#else
    YFResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The YFDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The YFDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        NSLog(@"The YFDatabase %@ is not open.", self);
        
#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The YFDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError*)errorWithMessage:(NSString *)message {
    ...
    return [NSError errorWithDomain:@"YFDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (YFResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
    return [self executeQuery:sql withArgumentsInArray:nil orDictionary:arguments orVAList:nil shouldBind:true];
}

- (YFResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args shouldBind:(BOOL)shouldBind {
    ...
    if (!statement) {
        statement = [[YFStatement alloc] init];
    ...
    rs = [YFResultSet resultSetWithStatement:statement usingParentDatabase:self shouldAutoClose:shouldBind];
    [rs setQuery:sql];
...
- (YFResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (YFResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (YFResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil shouldBind:true];
}

- (YFResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    YFResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil shouldBind:true];
    if (!rs && error) {
...
- (YFResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError * _Nullable __autoreleasing *)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    YFResultSet *rs = [self executeQuery:sql withArgumentsInArray:arrayArgs orDictionary:dictionaryArgs orVAList:args shouldBind:true];
    if (!rs) {
        if (outErr) {
...
int YFDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int YFDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) YFDBExecuteStatementsCallbackBlock)block {
    
    int rc;
    char *errmsg = nil;
    
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? YFDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
static NSString *YFDBEscapeSavePointName(NSString *savepointName) {
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError * _Nullable __autoreleasing *)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"savepoint '%@';", YFDBEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString*)name error:(NSError * _Nullable __autoreleasing *)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"release savepoint '%@';", YFDBEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString*)name error:(NSError * _Nullable __autoreleasing *)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"rollback transaction to savepoint '%@';", YFDBEscapeSavePointName(name)];
...
- (BOOL)checkpoint:(YFDBCheckpointMode)checkpointMode error:(NSError * __autoreleasing *)error {
    return [self checkpoint:checkpointMode name:nil logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(YFDBCheckpointMode)checkpointMode name:(NSString *)name error:(NSError * __autoreleasing *)error {
    return [self checkpoint:checkpointMode name:name logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(YFDBCheckpointMode)checkpointMode name:(NSString *)name logFrameCount:(int *)logFrameCount checkpointCount:(int *)checkpointCount error:(NSError * __autoreleasing *)error
{
...
void YFDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void YFDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
...
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (void*)b, &YFDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (__bridge void*)b, &YFDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}

- (YFSqliteValueType)valueType:(void *)value {
